...
object CreatureDao {
    ...
    suspend fun getCreaturesForEncounter(encounterId: Int): List<CoreCreature> = dbQuery {
        logger.debug { "Getting creatures for encounter $encounterId" }
        return@dbQuery Creatures.innerJoin(Encounterees)
            .slice(Creatures.columns)
            .select { Encounterees.encounterId eq encounterId }
            .let { CreatureDO.wrapRows(it) }.map { doToCoreCreature(it) }
    }
